...
> **本文档的使用方式**：把本文件放到鲁班猫4 的 ROS2 工作空间根目录（如 `~/wheeltec_ros2/src/CLAUDE.md`），
...
```bash
uname -m                  # 应为 aarch64
cat /etc/os-release       # 需要 Ubuntu 22.04 (jammy), ROS2 humble 的前提
ls /opt/ros/              # 应有 humble; 若没有: apt 安装 ros-humble-desktop 或 ros-humble-ros-base(推荐,省空间)
free -h                   # 4GB 内存确认
df -h /                   # 32GB 存储, 需保证 >=12GB 可用(源码2G+build/install~4G+swap6G)
ip link show              # 记下网卡名(RK平台一般是 eth0/eth1; WiFi 视型号)
whoami                    # 记下用户名(鲁班猫镜像常见 cat, 非 wheeltec!)
```
...
# 注意: 不要屏蔽 wheeltec_robot_slam 下的包以外的底盘基础包
...
# 若后续 colcon 报某个保留包缺依赖(依赖被屏蔽的包), 把对应 COLCON_IGNORE 删掉即可
```
...
# 2) FAST-LIO (模板重、单文件内存峰值高, 单独编, 源机7GB耗时7.5min, 本机预计10-20min)
...
# 同时 watch free -h 盯内存(FAST-LIO 建图占用应稳定在几百MB, 持续上涨说明分段保存失效)
...
# 检查 CPU/控制频率: 日志若频繁刷 "Control loop missed its desired rate of 20Hz"
